...
Requires:   python-%{service} = %{version}-%{release}
Requires:   python-%{service}-tests-gloang = %{version}-%{release}
...
%check
# octavia.tests.unit.cmd.test_agent test is failing so bypassing the check
...
%exclude %{python2_sitelib}/%{service}/tests/contrib/httpd.go
%endif

%files -n python-%{service}-tests-golang
%{python2_sitelib}/%{service}/tests/contrib/httpd.go
